...
Material::Material()
{
	...
	_shaderHeightSamplerLocation = 0;
...
bool Material::LoadShaders(std::string vertFilename, std::string fragFilename)
{
	...
	_shaderCameraLocation = glGetUniformLocation(_shaderProgram, "cameraPosition");
...
void Material::SetCameraPosition(glm::vec3* input)
{
	glUseProgram(_shaderProgram);
	glUniform3fv(_shaderCameraLocation, sizeof(input), (float*)input);
}
...
void Material::Apply()
{
	...
	glUniform1i(_shaderNormalSamplerLocation, 1);
